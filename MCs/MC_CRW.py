...
if __name__ == '__main__':
    ...
    # Save the time and probability arrays to a file for later plotting
        ...
        np.savez(outfile, t=t, prob_flux_dict=prob_flux_dict, prob_bare=prob_bare, seed=seed, necklaces=neck_strings, sequence=X, rngstate = rng_arr)
    ...
    #If continuing a calculation
        ...
        norm = len(old_necklaces) + len(neck_list)
        new_prob_flux_dict = {key: (len(neck_list)/norm* np.array(prob_flux_dict[key])+ len(old_necklaces)/norm* np.array(old_flux_dict[key])) for key in prob_flux_dict}     
